...
class Backend(AbstractBackend, metaclass=ABCDocstMeta):
    ...
    def _get_credentials(self):
        return oauth2client.client.OAuth2Credentials(access_token="1234", client_id=self.login,
                                                     client_secret=self.client_secret,
                                                     refresh_token=self.refresh_token,
                                                     token_expiry="0000-00-00T00:00:00Z", token_uri=Backend.TOKEN_URI,
    ...
    def is_temp_failure(self, exc):  # IGNORE:W0613
        if isinstance(exc, apiclient.errors.HttpError):
            ...
            if exc.resp.status == 403 and exc.resp.reason in ["Forbidden", "userRateLimitExceeded", "rateLimitExceeded",
                                                              "backendError"]:
                return True
            # Google best practice error says error 500 could be temporal, also We get some resumable errors with 400 Bad Request (Fully Random)
                ...
                return True
        elif isinstance(exc, (BrokenPipeError, ConnectionResetError, socket.gaierror, socket.herror, socket.timeout,
                              http.client.IncompleteRead,
                              httplib2.ServerNotFoundError, ssl.SSLZeroReturnError, ssl.SSLEOFError,
    ...
    def rename(self, src, dest, metadata=None, is_retry=False):
        ...
        log.debug("metadata: {0}, body: {1}".format(metadata, body))
        new_f = self.service.files().update(fileId=f['id'], body=body, addParents=target_folder['id'],
                                            removeParents=f['parents'][0]).execute()
        ...
        if self.buf.tell() >= self.buf.length:
            ...
            if self.done:
                ...
                if self.f['md5Checksum'] != self.md5.hexdigest():
                    log.warning('MD5 mismatch for %s: %s vs %s', self.f['title'], self.f['md5Checksum'],
                                self.md5.hexdigest())
                    raise BadDigestError('BadDigest', 'MD5 mismatch for %s (received: %s, sent: %s)' % (
                    self.f['title'], self.f['md5Checksum'], self.md5.hexdigest()))
                return b''
...
class ObjectW(object):
    ...
    def close(self, is_retry=False):
        ...
        self.fh.seek(0)
        binaryData = apiclient.http.MediaIoBaseUpload(self.fh, Backend.MIME_TYPE_BINARY, resumable=True,
                                                      chunksize=101 * 1024 * 1024)
        request = self.service.files().create(media_body=binaryData, body=self.metadata,
                                              fields='id,md5Checksum,name,properties')
        response = None
        while response is None:
            status, response = request.next_chunk()

        if len(self.metadata['properties']) > 0 and len(self.metadata['properties']) != len(response['properties']):
            self.rollbackUpload('metadata is invalid %s (received: %s, sent: %s)' % (
            self.key, response['properties'], self.metadata['properties']))
        if response['md5Checksum'] != self.md5.hexdigest():
            self.rollbackUpload(
                'MD5 mismatch for %s (received: %s, sent: %s)' % (self.key, checksumMD5, self.md5.hexdigest()))
